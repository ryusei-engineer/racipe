...
<form class="form_create" action="{{ route('recipe.store') }}" method="post" enctype="multipart/form-data">
  ...
  <input type="hidden" name="user_id" value="{{ \Auth::user()->id }}">
    ...
    <input type="file" name="image3" id="cooking_img3" value="{{ old('image3') }}">
      ...
      <input type="file" name="image4" id="cooking_img4" value="{{ old('image4') }}" accept="image/*">
        <div id="unselected4">
          <img src="{{ asset('img/camera-line.svg') }}" alt="">
          <span>写真を選択</span>
        </div>
...
手順5ハム、きゅうり、マヨネーズ、塩、胡椒を混ぜて完成">{{ old('procedure') }}</textarea>
  ...
  <div class="recipe_detail">
    ...
    調理時間
    <div>
      <input class="time" min="0" type="number" name="time" placeholder="10" value="{{ old('time') }}">
      <span>分</span>
    </div>
    @error('time')
    ...
    <select class="category_select" name="category_id">
        <option value="" {{ old('category_id') == 1 ? 'selected' : ''}}>カテゴリーを選択</option>
